 ...
 * Handles calls to the admin supplier crawler endpoints.
 *
 * Endpoints (per backend API contract):
 *   GET  /admin/supplier-categories   → category tree
...
// Crawl can perform N+1 fetches per category (listing + ~33 albums each, with
// polite 0.5–1.2s delays). 15 minutes leaves comfortable headroom.
const CRAWL_TIMEOUT_MS = 900_000;

/**
 * Fetch the supplier category tree from the backend.
 *
 * The backend fetches the upstream catalog (https://micom0078.x.yupoo.com/categories/)
 * and parses the HTML into a structured tree. Results are cached server-side
 * (1 hour).
 *
 ...
 * @property {string}        path             - URL path (e.g. /categories/5066922)
 ...
 */
export const getSupplierCategories = (refresh = false) =>
  apiClient
    .get('/admin/supplier-categories', {
    ...
    .then((res) => {
      ...
      const rawCategories = payload?.data?.categories ?? [];
 ...
 *  1. Fetch the album listing page
 *  2. For each album (~33 per category), fetch the album page with polite
 *     0.5–1.2s delays and parse images (.showalbum__imagecardwrap img[data-src])
 ...
 *   due to the N+1 fetch pattern. A 15-minute client timeout is set.
 *   Users should not close the tab.
 *
 ...
 * @property {number}       [durationMs] - Total wall-clock time on the server
 * @property {boolean}      [aborted]    - True if the crawl was cancelled mid-run
 *
